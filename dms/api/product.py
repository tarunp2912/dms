...
def set_role(team, user_id, role):
    ...
    dms_team = {k.user: k for k in frappe.get_doc("DMS Team", team).users}
    dms_team[user_id].is_admin = role
    dms_team[user_id].save()


@frappe.whitelist()
def remove_user(team, user_id):
    dms_team = {k.user: k for k in frappe.get_doc("DMS Team", team).users}
    if frappe.session.user not in dms_team:
        frappe.throw("User doesn't belong to team")
    frappe.delete_doc("DMS Team Member", dms_team[user_id].name)


@frappe.whitelist()
def get_all_users(team):
    team_users = {k.user: k.is_admin for k in frappe.get_doc("DMS Team", team).users}
