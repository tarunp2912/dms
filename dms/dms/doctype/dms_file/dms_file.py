...
class DMSFile(Document):
    ...
    def copy(self, new_parent=None, parent_user_directory=None):
        ...
        if self.is_group:
            ...
            dms_doc_content = frappe.db.get_list("DMS Document", self.document, "content")

            new_dms_doc = frappe.new_doc("DMS Document")
            new_dms_doc.title = title
            new_dms_doc.content = dms_doc_content
            new_dms_doc.save()
            ...
            dms_entity.flags.file_created = True
            dms_entity.insert()

        if new_parent == parent_user_directory.name:
            dms_entity.share(frappe.session.user, write=1, share=1)

        if dms_entity.mime_type:
            if dms_entity.mime_type.startswith("image") or dms_entity.mime_type.startswith(
                "video"
            ):
                frappe.enqueue(
                    create_thumbnail,
                    ...
                    entity_name=name,
                    path=path,
                    mime_type=dms_entity.mime_type,
                )
    ...
    def rename(self, new_title):
        ...
        :return: DMSEntity doc once it's renamed
        """
    ...
    def change_color(self, new_color):
        ...
        :return: DMSEntity doc once it's updated
        """
