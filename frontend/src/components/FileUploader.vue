...
function doesRootFolderFullPathExist(k, file_parent) {
  const url =
    ...
    "/api/method/" +
    `dms.api.files.does_entity_exist?name=${k}&parent_entity=${file_parent}`
...
function rootFolderFullPathNewName(k, file_parent) {
  const url =
    window.location.origin +
    "/api/method/" +
    `dms.utils.files.get_new_title?title=${k}&parent_name=${file_parent}&folder=${true}}`
...
onMounted(() => {
  dropzone.value = new Dropzone("div#dropzone", {
    ...
    retryChunks: true,
    forceChunking: true,
    url: "/api/method/dms.api.files.upload_file",
