...
export const getTeams = createResource({
  url: "/api/method/dms.api.permissions.get_teams",
  params: {
    details: 1,
  },
  method: "GET",
...
export const toggleFav = createResource({
  url: "dms.api.files.set_favourite",
  makeParams(data) {
    if (!data) {
...
export const clearRecent = createResource({
  url: "dms.api.files.remove_recents",
  makeParams: (data) => {
    if (!data) {
      getRecents.setData([])
      return { clear_all: true }
...
export const clearTrash = createResource({
  url: "dms.api.files.delete_entities",
  makeParams: (data) => {
    if (!data) {
      getTrash.setData([])
      return { clear_all: true }
...
export const createDocument = createResource({
  method: "POST",
  url: "dms.api.files.create_document_entity",
...
export const translate = createResource({
  method: "GET",
  url: "/api/method/dms.api.files.get_translate",
  cache: "translate",
})
